...
#### Hotspots Exploration -- ARTIFACT TYPES ####
...
plot1 = levelplot(at.hs.rast, col.regions = colpal, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of artifact hotspots"))
...
c.hs[['hotspot']] = rev(c("cores", "none", "recycled artifacts"))
c.hs[['order']] = (c(2,3,1))
c.hs = c.hs[order(-c.hs$order),]
levels(c.hs.rast) = c.hs
perc.c = as.data.frame(freq(c.hs.rast))
perc.c[['hotspot']] = c(rev(c("cores", "none", "recycled artifacts")), "NA")
perc.c$percentage = perc.c$count/cellStats(c.hs.rast, function(i, ...) sum(!is.na(i)))
perc.c$recycle.percentage = perc.c$count/perc.c$count[1]
perc.c$artifacts.percentage = perc.c$count/perc.c$count[3]
perc.c$overlap = "cores"
percentage = max(round(perc.c[which(perc.c$value == 1),]$artifacts.percentage*100, 2),0)
colpal.c = rev(c("#E69F00", "#0C7BDC", "grey80"))
plot4 = levelplot(c.hs.rast, col.regions = colpal.c, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of core hotspots"))
...
plot5 = levelplot(t.hs.rast, col.regions = colpal, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of tool hotspots"))

allperc = rbind(perc.at, perc.cf, perc.bf, perc.c, perc.t) %>% filter(value == 1)

tiff(filename = "figures/p1/artifact-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/complete_flakes-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/broken_flakes-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/cores-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/tools-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
plot1 = levelplot(sw.hs.rast, col.regions = colpal, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of strongly weathered artifact hotspots"))
plot(plot1)
...
plot2 = levelplot(mw.hs.rast, col.regions = colpal, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of mildly weathered artifact hotspots"))
plot(plot2)
...
plot3 = levelplot(ww.hs.rast, col.regions = colpal, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of weakly weathered artifact hotspots"))
plot(plot3)
...
nw.hs[['hotspot']] = rev(c("unweathered", "none", "recycled artifacts"))
nw.hs[['order']] = (c(2,3,1))
nw.hs = nw.hs[order(-nw.hs$order),]
levels(nw.hs.rast) = nw.hs
perc.nw = as.data.frame(freq(nw.hs.rast))
perc.nw[['hotspot']] = c(rev(c("unweathered", "none", "recycled artifacts")), "NA")
...
colpal.c = rev(c("#E69F00", "#0C7BDC", "grey80"))
plot4 = levelplot(nw.hs.rast, col.regions = colpal.c, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of unweathered artifact hotspots"))
plot(plot4)

allperc = rbind(perc.sw, perc.mw, perc.ww, perc.nw) %>% filter(value == 1)

tiff(filename = "figures/p1/strongly-weathered-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/mildly-weathered-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/weakly-weathered-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
...
tiff(filename = "figures/p1/unweathered-recycling-overlap.tiff", 
     units="in", 
     width=5*2, 
     height=4*2, 
     pointsize=12, 
