...
tiff(filename = "figures/p1/flakes-recycling-overlap.tiff",
     units="in",
     width=5*2,
     height=4*2,
     pointsize=12,
...
colpal.c = rev(c("#E69F00", "#0C7BDC", "grey80"))
plot4 = levelplot(nw.hs.rast, col.regions = colpal.c, main = paste0("recycled artifact hotspots overlap with ", percentage, "% of unweathered NR artifact hotspots"))
plot(plot4)

allperc.w = rbind(perc.sw, perc.mw, perc.ww, perc.nw) %>% filter(value == 1)
